...
(2) Momentum and size factors seem to work on Philippine stocks, with the latter generating the best annual rolling returns post-GFC. Size factor also seems to provide the best return after a market lull/downturn.
...
Note: Quintile portfolio is rebalanced every month. Returns are gross of trading costs.
...
n.quintile <- function(x, yr = 2019, q = 5){
  q.name = x %>% 
    filter(year(Date) == yr, multiple.quintile == paste0("Quintile ", q)) %>% 
...
mom.names <- mom.quintile %>% n.quintile()
vol.names <- vol.quintile %>% n.quintile()
per.names <- per.quintile %>% n.quintile()
mbr.names <- mbr.quintile %>% n.quintile()
mcap.names <- mcap.quintile %>% n.quintile()
```
