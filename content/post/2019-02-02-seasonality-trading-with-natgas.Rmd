...
Short: Feb, May-Jul, Nov (could NatGas's rally until end-Nov18 might be an outlier?)
...
Feeding the "seasonal" hypothesis, we quickly build a bayesian logistic regression to see if this pattern is statistically significant. Turns out it is not given the insignificant Pr(>|z|) values that the model spits out. I'll tests the trading signals derived from the model just to check how it would have performed historically. 
...
The model was still able to generate decent accuracy despite the insignificant Pr(>|z|) values.

```{r}
confusionMatrix(natgas.chg$m.ret, natgas.signal$glm.pred)
```

The models were still able to generate a decent equity curve except for the short-only strategy despite the statistically insignificance of the predictor variables (months and season). 
...
To date, the Bayesian GLM model is correct in its predictions as far as January 2019 is concerned. Despite its flaws, the model deserves further research given its potential as seen in its equity curve during backtesting before completely shelving it off. 
